...
- **发布流程合规修正**：
  - 在打发布 tag 之前完成中英文 changelog 同步
  - 将所有模块版本元数据更新收敛到一次合规发布步骤中

## [v1.0.27] - 2026-03-17

### 新增

- **ONNX Runtime 迁移与 Canary/Shadow 发布能力**：
  - 新增 ONNX 推理后端集成路径及 ML 服务运行时桥接能力
  - 新增 canary 与 shadow 评估流程，用于更安全的模型发布与可观测性验证
  - 新增 ONNX 推理辅助脚本与迁移说明文档

### 变更

- **按规则落地合规门禁流程**：
  - 新增阻断/告警合规脚本，并在 CI 中拆分为 `blocking_checks` / `advisory_checks`
  - 新增仓库级合规扫描命令，用于英文注释与规模阈值检查
  - 更新贡献流程，明确提交前执行顺序与混合门禁定义

### 修复

- **ML 推理接口一致性**：
  - 将 ONNX 结果构建从多参数调用调整为结构体入参，满足参数数量规则
  - 保持原有行为不变，并通过回归测试覆盖等价输出
...
- **代码规范对齐与可维护性强化**：
  - 完成全项目规则审计，按 `docs/CONTRIBUTING.md` 与 `docs/DEVELOPER_GUIDE.md` 对齐实现细节
  - 优化 Gateway 启动/运行时接线与 gRPC TLS 初始化流程，在不改变行为前提下降低代码复杂度
  - 统一命令入口与网关辅助逻辑的错误处理模式，满足更严格的 lint 和静态分析要求

### 修复

- **发布质量门禁**：
  - 清理当前 `golangci-lint` 阻塞项（静态错误、TLS 配置校验流程、风格/生命周期规则）
  - 完成 `go test ./...` 全量通过验证
