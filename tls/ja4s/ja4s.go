...
// AnalyzeServerHello 分析 TLS ServerHello 数据包
// serverHelloBytes: 完整的 ServerHello 字节数据
func (a *JA4SAnalyzer) AnalyzeServerHello(serverHelloBytes []byte) (*JA4SResult, error) {
...
// ComputeJA4S 便捷函数：直接从字节数据计算 JA4S
func ComputeJA4S(serverHelloBytes []byte) (*JA4SResult, error) {
	analyzer := NewJA4SAnalyzer()
	return analyzer.AnalyzeServerHello(serverHelloBytes)
}
