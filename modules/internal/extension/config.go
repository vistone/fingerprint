...
// NewUnifiedConfigFromEnv creates a unified configuration from environment variables
// This entry point loads both:
...
// applyRulesOverlay maps key thresholds from rules configuration to runtime configuration
...
// newLoggerConfig creates logger configuration presets
...
// newValidatorConfig creates validator configuration presets
func newValidatorConfig(env Environment) *ValidatorConfig {
	...
	switch env {
	case EnvProduction:
		config.MaxDataSize = 4096 // 4KB, strict limit
		config.StrictMode = true
	case EnvTesting:
		config.MaxDataSize = 65536 // 64KB
		config.StrictMode = true
	case EnvDevelopment:
		config.MaxDataSize = 1024 * 1024 // 1MB, relaxed limit
...
// newAuditConfig creates audit configuration presets
func newAuditConfig(env Environment) *AuditConfig {
	...
	case EnvDevelopment:
		config.Enable = false // Disabled by default in development environment
	}
