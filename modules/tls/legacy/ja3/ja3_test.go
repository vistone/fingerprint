...
// TestComputeJA3FromRealProfiles tests JA3 computation using real fingerprint profiles
func TestComputeJA3FromRealProfiles(t *testing.T) {
	...
	// Verify Hash is a valid MD5 (32 hexadecimal characters)
	if len(result.Hash) != 32 {
		t.Errorf("Expected MD5 hash length 32, got %d", len(result.Hash))
	}
}

// TestComputeJA3FromMultipleProfiles tests JA3 computation for multiple real fingerprints
...
// TestMatchJA3WithRealHashes tests matching using real JA3 hashes
func TestMatchJA3WithRealHashes(t *testing.T) {
	// Get JA3 hash of real fingerprint
	...
	// Test matching with same hash
	if !MatchJA3(realHash, realHash) {
		t.Error("Same hash should match")
	}

	// Test non-matching with different hash (modify one character)
...
// TestFindProfileByJA3WithRealHashes tests lookup using real JA3 hashes
func TestFindProfileByJA3WithRealHashes(t *testing.T) {
	// Compute JA3 for several real fingerprints
	...
	for _, profileName := range testProfiles {
		...
		// Look up profile using real hash
...
// TestValidateClientHelloSpec tests ClientHello spec validation
...
// TestComputeJA3FromSpecWithInvalidInput tests invalid input handling
func TestComputeJA3FromSpecWithInvalidInput(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: false, // Should skip nil extensions
		},
	}
...
// TestComputeJA3FromSpecEdgeCases tests edge cases
	...
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, result *JA3Result, err error) {
				...
				// Should correctly handle duplicate extensions
...
// TestIsGREASEValue tests GREASE value detection
func TestIsGREASEValue(t *testing.T) {
	tests := []struct {
		...
		{0xEAEA, true},
		{0xFAFA, true},
		{0x1301, false}, // Normal cipher suite
		{0x0000, false},
		{0xFFFF, false},
	}
...
// TestFilterGREASEFunctions tests GREASE filter functions
...
// BenchmarkComputeJA3FromSpec benchmark: JA3 computation performance
